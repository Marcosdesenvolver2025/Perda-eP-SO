# Ficha da loja — Google Play

Copie e cole cada bloco no Play Console em **Presença na loja → Ficha principal da loja**.
Idioma padrão: **português (Brasil) — pt-BR**.

---

## Nome do app (máx. 30 caracteres)

```
Vendas Itinga
```

## Descrição breve (máx. 80 caracteres)

```
Compre e venda em Itinga-MG com entrega local e 7 dias pra testar.
```

*(66 caracteres — o limite é 80)*

## Descrição completa (máx. 4.000 caracteres)

```
O Vendas Itinga é o app de compra e venda feito para Itinga-MG. Aqui você
anuncia o que não usa mais e encontra produtos novos e usados de gente que mora
pertinho de você — sem frete caro e sem esperar semanas pela entrega.

COMPRE COM SEGURANÇA
• 7 dias para testar: o prazo começa quando o produto chega na sua mão. Não
  gostou? Peça a devolução pelo app e receba de volta tudo que pagou, frete
  incluído — como garante o Código de Defesa do Consumidor.
• Pagamento protegido: o dinheiro fica retido com a gente e só é repassado ao
  vendedor depois que passa o prazo de teste.
• Pague com Pix ou cartão de crédito, parcelado em até 12x.

VENDA SEM COMPLICAÇÃO
• Anuncie de graça: tire as fotos, coloque o preço e pronto.
• Sem mensalidade. Você só paga quando vende.
• 16% de comissão quando você mesmo entrega.
• 18% quando o entregador do Vendas Itinga faz a coleta e a entrega.
• Receba direto na sua conta bancária.

ENTREGA DENTRO DA CIDADE
• Nossos entregadores buscam com o vendedor e levam até o comprador.
• Aceitamos pacotes de até 20 kg e 60 cm em cada lado.
• Acompanhe o pedido pelo app, do pagamento até a entrega.

TUDO SALVO NA SUA CONTA
• Entre com sua conta Google, sem senha nova para decorar.
• Suas compras, suas vendas e sua lojinha ficam guardadas.

O QUE DÁ PRA VENDER
Roupas, calçados, celulares, eletrônicos, móveis, eletrodomésticos, itens de
casa, coisas de bebê, bicicletas, ferramentas, livros, games e muito mais.

COMO FUNCIONA
1. Anuncie ou escolha um produto.
2. Pague por Pix ou cartão.
3. Receba pelo nosso entregador ou combine direto com quem vende.
4. Teste por 7 dias. Gostou? O vendedor recebe. Não gostou? Devolução integral.

Vendas Itinga: compre e venda na sua cidade.

Dúvidas ou sugestões: suporte@vendasitinga.com.br
```

---

## Categoria e etiquetas

| Campo | Valor |
|---|---|
| Tipo de app | Aplicativo |
| Categoria | Compras |
| Etiquetas | Compras, Classificados, Marketplace, Segunda mão |
| E-mail de contato | suporte@vendasitinga.com.br |
| Site | https://vendasitinga.com.br |
| Política de privacidade | https://vendasitinga.com.br/privacidade |

---

## Recursos gráficos

| Recurso | Tamanho | Arquivo pronto |
|---|---|---|
| Ícone do app | 512 × 512 PNG (32 bits, sem transparência) | `loja/graficos/icone-512.png` |
| Gráfico de destaque | 1024 × 500 PNG/JPG | `loja/graficos/capa-1024x500.png` |
| Capturas de tela do celular | mín. 2, máx. 8 — entre 320 px e 3840 px | **você precisa gerar** |

### Como tirar as capturas de tela

As capturas precisam ser do app rodando de verdade. Depois de instalar o APK
de teste no seu celular:

1. Abra o app e tire print das telas nesta ordem (dá uma boa história):
   1. Home com produtos
   2. Página de um produto
   3. Tela de "7 dias pra testar" / finalizar compra
   4. Aba vendas (a lojinha)
   5. Criar anúncio
   6. Acompanhamento do pedido
2. Envie no Play Console em **Ficha principal → Capturas de tela do smartphone**.

> A Google recusa capturas com moldura de celular falsa, texto exagerado cobrindo
> a interface ou imagem que não corresponde ao app. Print puro resolve.

---

## Textos de apoio (para a página de novidades a cada versão)

### Versão 1.0.0

```
Primeira versão do Vendas Itinga! Compre e venda produtos novos e usados dentro
da cidade, com entrega pelos nossos entregadores e 7 dias pra testar em casa.
```
